...
1. 申请邓白氏码【 DUNS 号】。苹果可以免费申请。
2. 收到 DUNS 号之后，继续完善公司信息。
3. 我们申请的时候是英文信息，如果需要显示中文公司名怎么办。在费用成功以后，我们可以向苹果公司发送邮件，申请修改公司名，但是修改的机会只有一次，确认无误后，即可在 appstore 显示中文公司信息。
...
编译出的程序将在哪个系统版本上运行，xcode8.0 之后最低版本只能选择 8.0，可以手动添加更低版本。为了最大程度的兼容，应用的 deployment target 决定了应用所能工作的设备系统的最低版本。

NOTE: 通过 svn 可以实现下载 github 项目的某一部分文件或者文件夹。选择要下载文件夹的目录，然后路径中”tree/master”改成”trunk”，使用 svn checkout 即可。
教程:http://www.jianshu.com/p/9beb55f23839
提高兼容性具体设置：Base SDK 选择 lastest IOS。Deployment Target 选择你所支持的最低版本。
设置更多的指令集
不同指令集的对应关系：
...
当前市面机型设置指令集参考：Architecture 的值选择：armv7 armv7s arm64。
三者的区别：

- architecture：想支持的指令集，armv7/armv7s/arm64
- valid architecture: 即将编译的指令集，实际编译的结果是两者的交集
- build active architectureonly：是否只编译当前设备适用的指令集，如果设为 yes，在 64 位系统会生成一个支持 64 指令集的 binary，一般 debug 为 yes，release 为 no，因为设置为 NO 的时候，会编译支持的所有版本，设置为 YES 的时候，是为 debug 的时候速度更快，只编译当前的 architecture 版本。
...
3. Target 指定了哪些内容?
   ...
   - Info
     - 这个 Info.plist 文件内定义了一个 iPhone 项目的很多关键性内容， 比如程序名称， 最终生成 product 的全局唯一 id 等等.每一个 Target 都能设置不同的
...
3. 请求存储到磁盘
4. 生成的 CertificateSigningRequest.certSigningRequest 的证书请求文件（ 注：CSR 文件尽量每个证书都制作一次，将常用名称区分开来，因为该常用名称是证书中的密钥的名字）

##### 建证书

在我们申请添加一个 Certificate 之前，需要先申请一个 Certificate Signing Request（CSR）文件，这个过程呢，实际上是生成了一对公钥和私钥，保存在我们电脑上的钥匙串中。代码的签名也就是使用这种基于非对称密钥的加密方式，用私钥进行签名，用公钥进行验证。  
...
注意：DistributionCertification 只有配置证书的电脑才可使用，因此即使导出导出 Keychain 安装到其他电脑上，其他电脑也不可能具有证书的权限。

## 发布

在 XCODE 中 archive 好，直接上传到苹果官网进行审核，填写资料虽然有点小麻烦，但是很有必要。

### Account 的配置

在 `Xcode -> Preferences` 下有 Account 的配置界面，点击 `+` 可以选择 Add Apple ID；必须登录之后才能在 xcode 使用相应的 PP 文件。点击 View Details 可以查看该 Apple Id 下的 Certificates 和 Provisioning Profile 证书文件，在这里你可以点击下载。

### 项目的配置

在项目 `TARGETS -> general`的 Identity 中， Team 选择对应的 Apple ID 即可。记得改变 build 和 version 对应的版本号.
...
- 上传 App Icon 的时候，需要上传 `1024*1024`的，而且不能有圆角效果哦
